...
<!-- End Menu -->
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h5><i class="bi bi-calendar4-range"></i>&nbsp;Planting Calendar / Edit Item</h5>
    ...
    <section class="section dashboard">
      <div class="row">
        ...
        $result = mysql_query("SELECT * FROM tblplantcalendar where fldcode='$vcode'");
        while($row = mysql_fetch_array($result))
        {
            $vcropcode=$row['fldcropcode'];
            $vtimeplantingall=$row['fldtimeplantingall'];                
            $vtimeplantingstart=$row['fldtimeplantingstart'];
            $vtimeplantingend=$row['fldtimeplantingend'];
            $vplantpopulation=$row['fldplantpopulation'];
            $vmaturitystart=$row['fldmaturitystart'];
            $vmaturityend=$row['fldmaturityend'];
            $vmaturityunit=$row['fldmaturityunit'];
            $vvolumestart=$row['fldvolumestart'];
            $vvolumeend=$row['fldvolumeend'];
            $vvolumeunit=$row['fldvolumeunit'];
            $vdistancestart=$row['flddistancestart'];
            $vdistanceend=$row['flddistanceend'];
        }
        $result = mysql_query("SELECT * FROM tblcrops where fldcode='$vcropcode' order by fldindex");
        while($row = mysql_fetch_array($result))
        {
            $vcrops=$row['fldcrops'];
        }
    ...
    $vctr=0;
        ...
        if($vtest==0)
        {
            if($vtestx==1)
            {
                ...
                mysql_query("UPDATE tblplantcalendar SET fldcropcode = '$vcropcodex',fldtimeplantingall = '$vtimeplantingallx',fldtimeplantingstart = '$vtimeplantingstartx',fldtimeplantingend = '$vtimeplantingendx',fldplantpopulation = '$vplantpopulationx',fldmaturitystart = '$vmaturitystartx',fldmaturityend = '$vmaturityendx',fldmaturityunit = '$vmaturityunitx',fldvolumestart = '$vvolumestartx',fldvolumeend = '$vvolumeendx',fldvolumeunit = '$vvolumeunitx',flddistancestart = '$vdistancestartx',flddistanceend = '$vdistanceendx'  WHERE fldcode = '$vcodex'");		
    ...
    ?>
            ...
            <form action="adminplantcalendaredit.php" name="frm" class="row g-3 needs-validation" id="block-validate" method="post" novalidate>
             <!--  <form class="row g-3 needs-validation" method="post" name="frm" id="block-validate" action="adminadminadd.php" enctype="multipart/form-data"  novalidate>-->
                ...
                {
                    $vcodex=$vcode;
                    $vcropcodex=$vcropcode;
                    $vcropsx=$vcrops;
                    $vtimeplantingallx=$vtimeplantingall;
                    if($vtimeplantingallx=="")
                    {
                        $vtimeplantingallx="Periodic";  
                    }
                    $vtimeplantingstartx=$vtimeplantingstart;
                    $vtimeplantingendx=$vtimeplantingend;
                    $vplantpopulationx=$vplantpopulation;
                    $vmaturitystartx=$vmaturitystart;
                    $vmaturityendx=$vmaturityend;
                    $vmaturityunitx=$vmaturityunit;
                    $vvolumestartx=$vvolumestart;
                    $vvolumeendx=$vvolumeend;
                    $vvolumeunitx=$vvolumeunit;
                    $vdistancestartx=$vdistancestart;
                    $vdistanceendx=$vdistanceend;
                }
                ...
                <div class="row mb-3">
                  ...
                  <div class="col-sm-3">
                  <input type="number" step="0.01" class="form-control col-sm-2" placeholder="0.0" id="validationCustom01" name="txtmaturitystart" value="<?php echo $vmaturitystartx; ?>" required>  
                    </div>
                    <div class="col-sm-3">
