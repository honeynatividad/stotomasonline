...
?>
    <div class="pagetitle">
      <h5><i class="bi bi-sliders"></i>&nbsp;Planting Seeds / Fertilizer / Record Fertilizer Application</h5>
      ...
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="farmplantseeds.php">Planting Seeds</a></li>
            ...
            <li class="breadcrumb-item">Record Fertilizer Application</li>
        </ol>
      </nav>
    ...
    <?php
        ...
        $vctr=0;
        $result = mysql_query("SELECT * FROM tblplantfert order by fldindex");
        while($row = mysql_fetch_array($result))
        {
            $vctr=$row['fldindex'];
        }
        ...
        if($vtest==0)
        {
            if($vtestx==1)
            {
                ...
                $vdistributioncode=$_POST['txtdistributioncode'];
                    ...
                    $sql="INSERT INTO tblplantfert (fldindex, fldcode, fldplantedcode, fldremarks, flddateapplied) VALUES ('$vctr','$vcodex','$vplantedcodex','$vremarksx','$vdateappliedx')";
                    ...
                    $result = mysql_query("SELECT * FROM tblplantfert where fldcode='$vcodex' order by fldindex");
                    while($row = mysql_fetch_array($result))
                    {
                        $vdateappliedx=$row['flddateapplied'];
                        $vplantcodex=$row['fldplantcode'];
                    }
                    $result = mysql_query("SELECT * FROM tblseedplanted where fldcode='$vplantedcodex' order by fldindex");
                    while($row = mysql_fetch_array($result))
                    {
                    ...
                    $vtitlex="Fertilizer Applied";
                    $vsubtitlex=$vuser." has applied fertilizer.";
                    $vnotificationx="Fertilizer has been applied on ".$vdateappliedx." on the ".$vcropx." seed planted on ".$vdateplantedx;
    ...
    ?>
            ...
            <form action="farmplantseedsfertadd.php" name="frm" class="row g-3 needs-validation" id="block-validate" method="post" novalidate>
             <!--  <form class="row g-3 needs-validation" method="post" name="frm" id="block-validate" action="adminadminadd.php" enctype="multipart/form-data"  novalidate>-->
                ...
                <div class="row mb-3">
                 ...
                 <div class="col-sm-8">
                  <textarea name="txtremarks" id="txtremarks"  rows="6" class="form-control" ><?php echo $vfeedback; ?></textarea>
                    </div>
                </div>
                ...
                <div class="row mb-3">
                  ...
                  <div class="col-sm-3">
                    <input type="date" class="form-control" name="txtdateapplied" required value=<?php echo date("Y-m-d"); ?>>
                  </div>
