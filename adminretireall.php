        ...
        if($vtest==0)
        {
            if($vtestx==1)
            {
				...
				while($row = mysql_fetch_array($result))
				{
                    ...
                    $arrharvestyear[$vi] = $vdatex->format("Y");  
                    $arrcode[$vi] = $row['fldcode'];
                    $arrdistributioncode[$vi] = $row['flddistributioncode'];
                ...
                while($vii<=$vi)
                {
                    ...
                    if($arrharvestyear[$vii]<$vyearnowx)
                    {
                    
                        mysql_query("UPDATE tblseedplanted SET fldstatus = 'Retired' WHERE fldcode = '$arrcode[$vii]'");		
                        
                        mysql_query("UPDATE tblseeddistribution SET fldstatus = 'Retired' WHERE fldcode = '$arrdistributioncode[$vii]'");	
                        
                        $result = mysql_query("SELECT * FROM tblseeddistribution where fldcode='$arrdistributioncode[$vii]' order by fldindex");
				        while($row = mysql_fetch_array($result))
				        {
                            $vrequestcode=$row['fldrequestcode'];
                        }
                        
                        mysql_query("UPDATE tblseedrequested SET fldstatus = 'Retired' WHERE fldcode = '$vrequestcode'");	
