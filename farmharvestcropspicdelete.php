...
if(isset($_SESSION['views']))
{
    ...
    $vpicharvestcodex=$_REQUEST['vuid'];
    ...
    $result = mysql_query("SELECT * FROM tblseedplanted where fldcode='$vseedplantedcodex'");
		while($row = mysql_fetch_array($result))
		{
    ...
    $result = mysql_query("SELECT * FROM tblseedplanted where fldcode='$vdistributioncodex'");
		while($row = mysql_fetch_array($result))
		{
    ...
    $result = mysql_query("SELECT * FROM tblseedrequested where fldcode='$vrequestcodex'");
		while($row = mysql_fetch_array($result))
		{
            $vrequestedseedcodex=$row['fldrequestedseed'];
           
            
 		}
    $result = mysql_query("SELECT * FROM tblcrops where fldcode='$vrequestedseedcodex'");
		while($row = mysql_fetch_array($result))
		{
    ...
    <!-- End Page Title -->
      <script>
        ...
        if($vtest==0)
        {
            if($vtestx==1)
            {
                ...
                mysql_query("DELETE FROM tblpicharvest WHERE fldcode = '$vcodex'");
                ?>
                <script>
      ...
      <div class="row align-items-top">
        <div class="col-lg-6">
          ...
          <div class="card">
            <div class="card-body">
            <form action="farmharvestcropspicdelete.php" name="frm" class="row g-3 needs-validation" id="block-validate" method="post" novalidate>
                <input type="hidden" name="txt1" id="txt1">
                    <input type="hidden" name="txtpost" id="txtpost">
                    <input type="hidden" name="txtcode" id="txtcode" value="<?php echo $vpicharvestcodex; ?>">
            <br>
            ...
            <div class="row mb-6">
                ...
                <div class="col-sm-8">
                  ...
                  <input type="text"  class="form-control col-sm-12" id="validationCustom01" readonly  name="txtdescription" value="<?php echo $vdescription; ?>" required>
                ...
                <div>
                    <button class="btn btn-danger btn-sm" type="submit" onClick="fSave1()">Delete</button>
                </div>
            </form>
            </div>
