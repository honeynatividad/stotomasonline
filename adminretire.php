...
<!-- End Menu -->
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h5><i class="bi bi-flower1"></i>&nbsp;Crops / Manage / Retire Transaction</h5>
    ...
    <section class="section dashboard">
      <div class="row">
        ...
        if($vtest==0)
        {
            if($vtestx==1)
            {
                $vrequestedcodex=$_POST['txtcode'];
                $vcodex=$_POST['txtcode'];
                $vcropcodex=$_POST['txtcropcode'];
                
                mysql_query("UPDATE tblseedrequested SET fldstatus = 'Retired' WHERE fldcode = '$vcodex'");		
                ...
                mysql_query("UPDATE tblseeddistribution SET fldstatus = 'Retired' WHERE fldcode = '$vcodex'");		
                
                $result = mysql_query("SELECT * FROM tblseedplanted where flddistributioncode='$vcodex'");
                while($row = mysql_fetch_array($result))
                {
                    $vcodex=$row['fldcode'];
             
                }
                mysql_query("UPDATE tblseedplanted SET fldstatus = 'Retired' WHERE fldcode = '$vcodex'");		
                ?>
                <script>
								    alert("Transaction Retired.");
								    </script>   
    		                                         
                <meta  http-equiv="refresh" content=".000001;url=adminretire.php?vuid=<?php echo $vusercode; ?>&vuid1=<?php echo $vcropcodex; ?>&vuid2=<?php echo $vrequestedcodex; ?>" />
              <?php  
            }
        ...
        ?>  
        <form action="adminretire.php" name="frm" class="row g-3 needs-validation" id="block-validate" method="post" novalidate>
        
                <input type="hidden" name="txt1" id="txt1">
                    <input type="hidden" name="txtpost" id="txtpost">
                    <input type="hidden" name="txtcode" id="txtcode" value="<?php echo $vrequestedcode; ?>">
                    <input type="hidden" name="txtcropcode" id="txtcropcode" value="<?php echo $vcropcode; ?>">
        <!-- Left side columns -->
        <div class="col-lg-12">
          <div class="row">
            <div class="col-lg-12">
            ...
            <div class="card-body">
              ...
              <table class="table datatable">
                ...
                <tbody>
                     <?php 
				        $vcounter=1;
                        $result = mysql_query("SELECT * FROM tblseedrequested where fldcode='$vrequestedcode' order by fldindex");
				        while($row = mysql_fetch_array($result))
				        {
                  ...
                  <tr>
                    ...
                    <td><button type="button" class="btn btn-warning btn-sm" onclick="window.location.href='adminseeddistribution.php?vuid=<?php echo $vcropcode; ?>'">Return</button></td> 
                    
                    <td>
                        <?php
                        if($row['fldstatus']!="Retired")      
                        {
                        ?>
                        <button class="btn btn-danger btn-sm" type="submit" onClick="fSave1()">Confirm</button>
                        ...
                        {
                        ?>
                        <button disabled class="btn btn-danger btn-sm" type="submit" onClick="fSave1()">Confirm</button>
                        <?php
                        }
                        ?>
                      </td> 
