...
<!-- End Menu -->
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h5><i class="bi bi-vinyl"></i>&nbsp;Seeds / Add New Seed Request</h5>
     
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="farmdashboard.php">Seeds</a></li>
            <li class="breadcrumb-item">Add New Seed Request</li>
        </ol>
      </nav>
    ...
    <?php
        ...
        $result = mysql_query("SELECT * FROM tblcrops where fldcode='$vrequestedseedx' order by fldindex");
        while($row = mysql_fetch_array($result))
        {
            $vcropx=$row['fldcrops'];
        }
        ...
        $result = mysql_query("SELECT * FROM tbluser where fldcode='$vusercode' && fldlocation='$vlocationx' order by fldindex");
        while($row = mysql_fetch_array($result))
        {
            $vlotareax=$row['fldlotarea'];
        }
        $result = mysql_query("SELECT * FROM tblfarm where fldfarmercode='$vusercode' && fldlocation='$vlocationx' order by fldindex");
        while($row = mysql_fetch_array($result))
        {
            $vlotareax=$row['fldlotarea'];
        }
        ...
        if($vtest==0)
        {
            if($vtestx==1)
            {
                ...
                $splitdata = explode("-",$vlocationx);
                $farmcode = $splitdata[1];
                $newvlocationx = $splitdata[0];
                ...
                $vdaterequestedx = $vdatex->format("Y-m-d");	
                    ...
                    $sql="INSERT INTO tblseedrequested (fldindex, fldcode, fldrequestedby, fldlocation, fldrequestedseed, flddaterequested, fldrequestedqty, fldstatus, fldfarmcode) VALUES ('$vctr','$vcodex','$vrequestedbyx','$newvlocationx','$vrequestedseedx','$vdaterequestedx','$vrequestedqtyx','Seed Requested','$farmcode')";
                    ...
                    $vtitlex="Seed Request";
                    $vsubtitlex=$vuser." submitted a seed request.";
                    $vnotificationx="The following are the seed requests: ".$vcropx."-".$vrequestedqtyx."(".'Grams'.")";
                    ...
                    while($row = mysql_fetch_array($result))
                    {
                        ...
                        ////////////////////////////////////email
                                        ...
                                        $message = 'Dear '.$vfirstnamex."\r\n"."\r\n".'You have a seed request from '.$vusername.'.'."\r\n"."\r\n".'Thank you.';
    ...
    ?>
            ...
            <form action="farmseedreqadd.php" name="frm" class="row g-3 needs-validation" id="block-validate" method="post" novalidate>
             <!--  <form class="row g-3 needs-validation" method="post" name="frm" id="block-validate" action="adminadminadd.php" enctype="multipart/form-data"  novalidate>-->
                ...
                <div class="row mb-3">
                  ...
                  <div class="col-sm-8">
                    <select id="inputState" class="form-select" name="txtrequestedseed">
                      ...
                      {
                      ?>
                      <option value="<?php echo $vrequestedseedx; ?>"><?php echo $vcropx; ?></option>  
                      <?php
                      }
                      ?>
                      <?php
                ...
                <div class="row mb-3">
                  <label for="validationCustom01" class="form-label col-sm-2 col-form-label">Seed Quantity (Grams)</label>
                  <div class="col-sm-3">
                  <input type="number" step="0.01" class="form-control col-sm-2" placeholder="0.0" id="validationCustom01" name="txtrequestedqty" value="<?php echo $vrequestedqtyx; ?>" required>  
                    </div>
                ...
                <div class="row mb-3">
                  ...
                  <div class="col-sm-8">
                    <select id="inputState" class="form-select" name="txtlocation">
                      ...
                      ?>
                      <option value="<?php echo $fldlocation; ?>-<?php echo $rowfarm['fldcode'] ?>"><?php echo $fldlocation; ?></option>  
                      <?php
                      }
                      ?>
                    </select>
