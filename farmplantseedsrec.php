...
<!-- End Menu -->
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h5><i class="bi bi-sliders"></i>&nbsp;Planting Seeds / Record</h5>
    ...
    <?php
        ...
        $result = mysql_query("SELECT * FROM tblseeddistribution where fldrequestcode='$vcodex'");
        while($row = mysql_fetch_array($result))
        {
            $vdistributioncodex=$row['fldcode'];
        ...
        if($vtest==0)
        {
            if($vtestx==1)
            {
                ...
                $vdistributioncodex=$_POST['txtdistributioncode'];
                ...
                $vexisting=0;
                $result = mysql_query("SELECT * FROM tblseedplanted where flddistributioncode='$vdistributioncodex'");
                while($row = mysql_fetch_array($result))
                {
                    $vexisting=1;
                }
                ...
                {
                    $vctr=0;
                    $result = mysql_query("SELECT * FROM tblseedplanted order by fldindex");
                    while($row = mysql_fetch_array($result))
                    {
                        $vctr=$row['fldindex'];
                    }
                    ...
                    $vtitlex="Seed Planted";
                    $vsubtitlex=$vuser." has planted seeds.";
                    $vnotificationx="The following are the seeds planted: ".$vcropx."-".$vseedplantedqtyx."(".$vunitx.")";
                ...
                else
                {
                    ...
                    $vtitlex="Seed Planted";
                    $vsubtitlex=$vuser." edited the number of seeds planted.";
                    $vnotificationx="The following are the new seeds planted: ".$vcropx."-".$vseedplantedqtyx."(".$vunitx.")";
                    
                    $vselectdate=date();
                    $vdatex = new DateTime($vselectdate);
                    $vdate1x = $vdatex->format("Y-m-d");	    
                
                    $sql="INSERT INTO tblnotification (fldindex, fldcode, fldtitle, fldsubtitle, fldnotification, flddate, fldstatus, fldsource) VALUES ('$vctr','$vcodex','$vtitlex','$vsubtitlex','$vnotificationx','$vdate1x','Not Yet Viewed','Farmer')";
                    if (!mysql_query($sql,$con))
				    {
  						die('error: ' . mysql_error());
  		            }
                    
                     mysql_query("UPDATE tblseedplanted SET flddateplanted = '$vdateplantedx' WHERE flddistributioncode = '$vdistributioncodex'");		
                    
                      
              
                }
                
                mysql_query("UPDATE tblseeddistribution SET fldstatus = 'Seed Planted' WHERE fldcode = '$vdistributioncodex'");		
                ...
                $result = mysql_query("SELECT * FROM tbluser where fldusertype!='Farmer' order by fldindex");
                    while($row = mysql_fetch_array($result))
                    {
                        ...
                        ////////////////////////////////////email
                                        $to      = $vemailx;
                                        $subject = 'Seed Planted';
                                        $message = 'Dear '.$vfirstnamex."\r\n"."\r\n".$vusername.'has planted seeds.'."\r\n"."\r\n".'Thank you.';
                 ...
                 ?>
                    <script>
				    alert("Planted Seed Recorded.");	
				    </script>   
    		          <meta  http-equiv="refresh" content=".000001;url=farmplantseeds.php" />
            	       <?php   
            }
        }
    /////////////////////////////////////////////////////////
    ?>
            ...
            <form action="farmplantseedsrec.php" name="frm" class="row g-3 needs-validation" id="block-validate" method="post" novalidate>
             <!--  <form class="row g-3 needs-validation" method="post" name="frm" id="block-validate" action="adminadminadd.php" enctype="multipart/form-data"  novalidate>-->
                ...
                <input type="text"  class="form-control col-sm-6" id="validationCustom01" readonly name="txtcrops" value="<?php echo $vcropsx; ?>" required>
                ...
                <div class="row mb-3">
                  ...
                  <div class="col-sm-3">
                  <input readonly type="number" step="0.01" class="form-control col-sm-2" placeholder="0.0" id="validationCustom01" name="txtseedplantedqty" value="<?php echo $vrequestedqtyx; ?>" required>  
                    </div>
                
                    <label for="inputDate" class="col-sm-2 col-form-label">Date Planted</label>
                  <div class="col-sm-3">
                    <input type="date" class="form-control" name="txtdateplanted" required value=<?php echo date("Y-m-d"); ?>>
                  </div>
                ...
                <div class="col-12">
                  ...
                  <button class="btn btn-warning" type="button" onclick="window.location.href='farmplantseeds.php'">Return</button>
                </div>
