...
<!-- End Menu -->
  ...
  <main id="main" class="main">
    ...
    <?php
        ...
        $result = mysql_query("SELECT * FROM tblseedrequested where fldcode='$vcode'");
        while($row = mysql_fetch_array($result))
        {
            $vcropcode=$row['fldrequestedseed'];
            $vrequestedqty=$row['fldrequestedqty'];
            $vrequestedseed=$row['fldrequestedseed'];
            ...
            $vdaterequested = $vdatex->format("Y-m-d");	

        }
        $result = mysql_query("SELECT * FROM tblcrops where fldcode='$vrequestedseed' order by fldindex");
        while($row = mysql_fetch_array($result))
        {
            $vcrop=$row['fldcrops'];
        }
        ...
        $result = mysql_query("SELECT * FROM tblfarm where fldfarmercode='$vusercode' && fldlocation='$vlocation'");
        while($row = mysql_fetch_array($result))
        {
            $vlotarea=$row['fldlotarea'];
        }
        ...
        if($vtest==0)
        {
            if($vtestx==1)
            {
                ...
                */
                    ...
                    mysql_query("UPDATE tblseedrequested SET fldrequestedseed = '$vrequestedseedx',fldrequestedqty = '$vrequestedqtyx',fldlocation = '$vlocationx' WHERE fldcode = '$vcodex'");		
    ...
    ?>
            ...
            <form action="farmseedreqedit.php" name="frm" class="row g-3 needs-validation" id="block-validate" method="post" novalidate>
             <!--  <form class="row g-3 needs-validation" method="post" name="frm" id="block-validate" action="adminadminadd.php" enctype="multipart/form-data"  novalidate>-->
                ...
                <div class="row mb-3">
                 ...
                 <div class="col-sm-8">
                  <select id="inputState" class="form-select" name="txtrequestedseed">
                    ...
                    {
                    ?>
                    <option value="<?php echo $vrequestedseed; ?>"><?php echo $vcrop; ?></option>  
                    <?php
                    }
                    ?>
                    <?php
                ...
                {
                ?>
                  <input type="number" step="0.01" class="form-control col-sm-2" placeholder="0.0" id="validationCustom01" name="txtrequestedqty" value="<?php echo $vrequestedqty; ?>" required>  
                <?php
                }
                ?>
                    </div>
